...
const DataPribadiSection = () => {
      ...
      try {
        ...
        if (res.data) {
          ...
          if (res.data.masaBerlakuPaspor) {
            const match = res.data.masaBerlakuPaspor.match(
        ...
        if (err.response && err.response.status === 404) {
          console.warn("⚠️ CV belum dibuat, user baru login");
        } else {
          console.error("❌ Error ambil CV:", err);
        }
      }
    };
    fetchCV();
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/cv`, dataToSend, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-3 bg-red-100 border border-red-300 text-red-700 rounded-md text-sm font-semibold">
        ⚠️ Semua kolom wajib diisi. Kecuali data paspor jika belum punya.
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              * File foto maksimal 15MB (disarankan JPG/PNG)
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          <label htmlFor="nama" className="mb-1 font-semibold text-gray-700">
            Nama Lengkap
          </label>
          <input
            id="nama"
        ...
        <div className="flex flex-col">
          ...
          {form.tanggalLahir && (
            <div className="text-sm text-gray-500 mt-1">
              Format Jepang: {formatJapaneseDate(form.tanggalLahir)}
            </div>
          )}
        </div>
        ...
        {/* Usia */}
        <div className="flex flex-col">
          <label htmlFor="usia" className="mb-1 font-semibold text-gray-700">
            Usia (Otomatis)
          </label>
          <input
            id="usia"
        ...
        {/* No. Telepon */}
        <div className="flex flex-col">
          <label htmlFor="telp" className="mb-1 font-semibold text-gray-700">
            No. Telepon
          </label>
          <input
            id="telp"
        ...
        {/* Agama */}
        <div className="flex flex-col">
          <label htmlFor="agama" className="mb-1 font-semibold text-gray-700">
            Agama (Katakana)
          </label>
          <select
            id="agama"
        ...
        {/* Konsumsi Alkohol */}
        <div className="flex flex-col">
          <label htmlFor="alkohol" className="mb-1 font-semibold text-gray-700">
            Konsumsi Alkohol
          </label>
          <select
            id="alkohol"
        ...
        {/* Merokok */}
        <div className="flex flex-col">
          <label htmlFor="merokok" className="mb-1 font-semibold text-gray-700">
            Merokok
          </label>
          <select
            id="merokok"
        ...
        <div className="flex flex-col">
          ...
          {form.masaBerlakuPasporJp && (
            <div className="text-sm text-gray-500 mt-1">
              Format Jepang: {form.masaBerlakuPasporJp}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col md:col-span-2">
          <label
            ...
            className="mb-1 font-semibold text-gray-700"
          >
            自己PR (Promosi Diri dalam bahasa Jepang. Singkat saja)
          </label>
          <div className="text-sm text-gray-500 mb-2 mt-1">
            Contoh:
            状況の判断がうまく出来る人です。また、責任のある人です。お仕事は丁寧で、テキパキと行動することが出来ます。
          </div>
          <textarea
            id="promosiDiri"
            name="promosiDiri"
      ...
      <div className="p-3 bg-yellow-100 border border-yellow-300 text-yellow-700 rounded-md text-sm font-semibold">
        ⚠️ Simpan halaman Data Pribadi ini sebelum lanjut ke halaman berikutnya.
      </div>
